...
-- custom function to get all users if claims_admin
CREATE OR REPLACE FUNCTION get_all_users() RETURNS SETOF auth.users
    LANGUAGE "plpgsql" SECURITY DEFINER SET search_path = public
    AS $$
    DECLARE retval auth.users%rowtype;
    BEGIN
      IF NOT is_claims_admin() THEN
        RETURN;
      ELSE
        FOR retval in SELECT * FROM auth.users ORDER BY auth.users.id loop
          return next retval;
        END loop;
      END IF;
    END;
